...
const AdminList = ({ leader, setLeader }) => {
      ...
      <Listbox  value={selectedAdmin} onChange={handleChange} className="h-full " multiple>
        <div className=''>
            <div className={`h-full flex justify-center items-center flex-wrap ${leader.length > 0 ? '' : 'hidden'}`}>
              {leader.length > 0 ? selectedAdmin.map((admin, index)=> admin.image ? (<img key={index} src={admin.image} alt="" className={`${leader.length == 1 ? 'w-8 h-8': leader.length == 2 ? 'w-6 h-6' : 'w-4 h-4' } rounded-full border border-black`} />) : <div className={`${leader.length == 1 ? 'w-8 h-8': leader.length == 2 ? 'w-6 h-6' : 'w-4 h-4' } rounded-full border bg-purple-500 text-white flex items-center justify-center`}><span>{getInitials(admin.name)}</span></div>)  : null}
            </div>
                  ...
                  <div>
                    <div className={clsx('flex items-center gap-2  truncate', selected ? 'font-medium' : 'font-normal')}>
                      <div className="w-6 h-6 rounded-full text-white flex items-center justify-center bg-violet-600">
                        {user?.image?.length > 1 ? <img className='w-full h-full rounded-full border-2 border-gray-300 shadow-md object-cover' src={user?.image} alt='' /> :<span className="text-center text-[10px]">{getInitials(user.name)}</span>}
