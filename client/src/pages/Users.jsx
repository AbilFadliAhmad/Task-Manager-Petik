...
import { AddUser, Button, ConfirmationDialog, Loading, Title } from '../components';
...
const Users = () => {
  ...
  const [statusAccount] = useStatusMutation();
  ...
  const userActionHandler = async () => {
    ...
    try {
      await statusAccount({ id: selected._id, isActive: !selected.isActive }).unwrap();
      toast.dismiss(l);
      toast.success(`User bernama ${selected?.name} berhasil ${selected.isActive ? 'dinonaktifkan' : 'di Aktifkan Kembali'}`);
      setTimeout(() => {
        setOpenAction(false);
        refetch();
        setLoading(false);
  ...
  const deleteHandler = async () => {
    ...
    try {
      ...
      toast.dismiss(l);
      toast.success(`User Bernama ${selected?.name} berhasil dihapus`);
      setTimeout(() => {
        setOpenDialog(false);
        refetch();
        setLoading(false);
      ...
      setLoading(false);
      console.log(error);
      toast.error(`User ${selected?.name} gagal Dihapus`);
    }
  };
  ...
  const TableRow = ({ user }) => (
    <tr
      className={`seamlessly ${theme.darkMode ? 'border-white text-white hover:bg-gray-400/20' : 'border-gray-300 text-gray-600 hover:bg-gray-400/50'} border-b  ${
        user.isAdmin ? `${theme.darkMode ? 'bg-yellow-600' : 'bg-yellow-200'}` : user.isUstadz ? 'bg-blue-500/20' : ''
      }  text-sm sm:text-lg`}
    >
      <td className="p-2">
        <div className="flex items-center gap-3">
          <div className={`w-11 h-11 rounded-full flex items-center justify-center ${user.image ? 'border-2 border-gray-300  sm:text-sm' : 'text-sm bg-blue-700 text-white'}`}>
            {user.image ? <img src={user.image} alt="" className="w-full h-full rounded-full object-cover" /> : <span className="text-xs md:text-sm text-center">{getInitials(user?.name)}</span>}
          </div>
          <span>{user?.name}</span>
        </div>
      </td>
      ...
      <td className="p-2 sm:p-0">
        <button onClick={() => userStatusCLick(user)} className={`w-fit px-4 py-1 rounded-full disabled:cursor-not-allowed text-white ${user?.isActive ? 'bg-blue-500' : 'bg-red-500'}`} disabled={user.isAdmin}>
          {user?.isActive ? 'Active' : 'Disabled'}
        </button>
      </td>

      <td className="p-2 flex gap-4">
        <Button label="Edit" type={'button'} className={`${theme.darkMode ? 'text-blue-100 hover:text-blue-500 font-bold' : 'text-blue-600 hover:text-blue-500'} sm:px-0 text-sm md:text-base`} onClick={() => editClick(user)} />
  ...
  return (
    <>
      <div className="w-full md:p-4 px-0 mb-6">
        <div className="flex items-center justify-between mb-8">
          ...
          <Button
            ...
            className={'flex flex-row-reverse gap-1 items-center bg-blue-500 text-white text-sm sm:text-lg w-[9rem] sm:w-[10.32rem]'}
      ...
      <UserAction isLoading={loading} theme={theme} open={openAction} setOpen={setOpenAction} onClick={userActionHandler} status={selected?.isActive} />
    </>
  );
};
