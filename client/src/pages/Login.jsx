...
const Login = () => {
  ...
  return (
    <div className={`w-full min-h-screen flex items-center justify-center flex-col lg:flex-row overflow-scroll ${theme?.darkMode ? 'bg-gray-900 ' : 'bg-[#f4f5f6]'} `}>
      <div className="w-full md:w-auto flex gap-0 md:gap-40 flex-col md:flex-row items-center justify-center">
        ...
        <div className='w-full md:w-1/3 p-4 md:p-1 fkex flex-col justify-center items-center'>
          <form  onSubmit={handleSubmit(submitHandler)} className={`form-container w-full md:w-[400px] flex flex-col gap-y-8 ${theme?.darkMode ? 'bg-gray-800' : 'bg-white'}  px-10 pt-14 pb-14`}>
            <div className=''>
              <p className='text-green-500 text-2xl font-bold text-center'>Selamat Datang Kembali</p>
              <p className={`text-center text-base ${theme?.darkMode ? 'text-neutral-300' : 'text-gray-700'} `}>Jaga kredensial anda tetap aman</p>
            </div>
            ...
            <Textbox
                ...
                theme={theme}
              />
              <Link to={'/admin-support'} className={`text-sm ${theme?.darkMode ? 'text-neutral-200/80 hover:text-blue-400' : 'text-gray-500 hover:text-blue-600'}  hover:underline cursor-pointer`}>
                Hubungi Admin?
              </Link>
