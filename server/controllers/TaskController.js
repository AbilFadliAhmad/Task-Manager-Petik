  ...
  try {
    ...
    if (leader?.length > 0) {
      ...
      if (leader.length == 1) {
        ...
        text1[0] = `Admin telah menunjuk anda ${
          array.length - 1 > 0 ? `dan ${array?.length - 1} User yang lain ` : ''
        } untuk memimpin tugas ini. Silakan pilih beberapa anggota tambahan untuk mendukung penyelesaian tugas ini dengan lebih efektif`;
        task.leader = [...array];
        notif.leader = [...array];
      }
    ...
    await historyModel.create({
      ...
      textLog: `Tugas baru dengan judul: ${title} terlah berhasil dibuat oleh ${user.name}, tugas ini disetting di level ${priority}, tugas ini ditetapkan untuk ditangani pada ${tanggalIndo(
        tanggal
      )}. Pemimpin dari tugas ini terdiri dari ${semuaPemimpin.join(', ')}`,
    });
...
const isExpiredTask = async (req, res) => {
  try {
    const { id, isExpired, blink } = req.body;
    const task = await taskModel.findById(id);
    task.isExpired = isExpired;
    task.blink = blink ? true : false;
    await task.save();
...
const duplicateTask = async (req, res) => {
  try {
    ...
    await historyModel.create({
      ...
      textLog: `Tugas baru dengan judul: ${newTask.title} terlah berhasil diduplikat oleh ${user.name}, tugas ini disetting di level ${newTask.priority}, tugas ini ditetapkan saat ${tanggalIndo(
        tanggal
      )}. Pemimpin dari tugas ini terdiri dari ${semuaPemimpin.join(', ')}`,
    });
...
const dashboardStatistics = async (req, res) => {
  try {
    ...
    const blinkTasks = allTasks.filter((task) => task.blink);
...
const updateTask = async (req, res) => {
  try {
    ...
    task.deadline = deadline.length > 0 && timer ? deadline : deadline.length > 0 ? deadline : task.deadline;
    console.log(timer, 'waktu dari server');
    console.log(deadline, 'deadline dari server');
    task.date = date;
    ...
    await historyModel.create({
      ...
      ${
        !_.isEqual(dataHasilPopulatePrevTask.team, dataNewTask.team)
          ? `mengubah anggota dari ${dataHasilPopulatePrevTask.team.map((user) => user.name).join(`, `) ?? 'tidak ada'} menjadi ${taskBaru.team.map((user) => user.name).join(`, `)}, `
          : ``
      } 
      ${
        !_.isEqual(dataHasilPopulatePrevTask.leader, dataNewTask.leader)
          ? `mengubah pemimpin dari ${dataHasilPopulatePrevTask.leader.map((user) => user.name).join(`, `) ?? 'tidak ada'} menjadi ${taskBaru.leader.map((user) => user.name).join(`, `)}, `
          : ``
      } 
      ${dataPrevTask.priority !== priority.toLowerCase() ? `Level ${dataPrevTask.priority} menjadi Level ${priority.toLowerCase()}, ` : ``} 
      ${dataPrevTask.stage !== stage.toLowerCase() ? `Level ${dataPrevTask.stage} menjadi Level ${stage.toLowerCase()}, ` : ``}`
        .replace(/\n\s*\n/g, '\n')
        .replace(/\s+/g, ' ')
        .trim(),
    });
...
const deleteRestoreTask = async (req, res) => {
  try {
    ...
    if (actionType === 'delete') {
      ...
      await taskModel.updateMany({ isTrashed: true }, { $set: { isTrashed: false } });
...
const listTasks = async (req, res) => {
  ...
  if(arrayItem?.length > 0) {
    ...
    const timer = path == 'timer' ? true : false;
